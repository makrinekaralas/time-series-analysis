...
from tsf_bmw.imdict import imdict
...
class UVariateTimeSeriesForecaster ( object ):
    ...
    def __init__(self, ts_dict):
        ...
        Initializes the object UVariateTimeSeriesForecaster
             ts_dict - a dictionary that initializes the class. It has to be of the structure
                       {'ts_df': ts_df, 'time_format': '%Y-%m-%d %H:%M:%S.%f', 'freq': freq, 'transform': transform, 'p_train': p_train} with
                       ...
                       transform - the transformation that will be applied on time series (right now log10 and box-cox supported)
        ...
        try:
            ...
            print ( "Time series must be univariate. Hence, at most a time columns and a column of numeric values are expected!" )
            sys.exit ( "STOP" )
        else:
    ...
    def ts_check_frequency(self):
        ...
        if self.ts_df.index.freq is None:
            ...
            print ( "Frequency chosen in initialization: " + str(self._freq) + " enter 'n' and call ts_resample() if you are satisfied with this value." )  
    ...
    def ts_fit(self,
        ...
        Parameters:
            ...
            - suppress: suppresses output messages, right now only when method == 'lin'
        """
            ...
            try:
                ...
                if self.model_params['diagnose']:
                        try:
                            ...
                            print (
                                "You want to diagnose the Prophet model. Please provide parameters 'period' and 'horizon' within object initialization!" )
                            sys.exit ( "STOP " )
                ...
                if not dict_params['consider_holidays']:
                    self.model['m'] = Prophet ( interval_width=dict_params['interval_width'],
                        ...
                        try:
                            ...
                            print (
                                "You need to consider holidays? Then provide country name! Right now, Austria (AT), Germany(DE) and USA (US) supported." )
                            sys.exit ( "STOP" )
                        else:
...
def main():
    ...
    if args.method == 'prophet':
        ...
        if input ( "Would you like to diagnose prophet (cross validation) y/n?" ).strip ().lower () == 'y':
            history = input (
                "Enter history of format, e.g., '725 days' or '10000 hours'. Empty value would mean, 3X horizon will be considered: ".strip ().lower () )
            if history == '':
